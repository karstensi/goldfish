...
func GetHCLFilesFromPath(accessToken, owner, repo, branch, path, base, head string) (map[string]string, error) {
	if accessToken == "" || owner == "" || repo == "" || head == "" {
		return nil, errors.New("Config_path does not include gitlab info required")
	}

	// construct oauth gitlab client from personal access token
	ctx := context.Background()
	ts := oauth2.StaticTokenSource(
		&oauth2.Token{AccessToken: accessToken},
	)
	client := gitlab.NewClient(oauth2.NewClient(ctx, ts))
	...
	_, folder, _, err := client.Repositories.GetContents(ctx, owner, repo, path,
		&gitlab.RepositoryContentGetOptions{Ref: head},
	)
	if err != nil {
		return nil, err
	}
