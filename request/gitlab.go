...
import (
	...
	"github.com/karstensi/goldfish/gitlab"
...
func (r GitlabRequest) IsRootOnly() bool {
	return true
}

// verifies user can read all policies in the changes
func CreateGitlabRequest(auth *vault.AuthInfo, raw map[string]interface{}) (*GitlabRequest, error) {
	r := &GitlabRequest{
	...
	newPolicies, err := github.GetHCLFilesFromPath(
		...
		conf.GitlabRepoOwner,
		conf.GitlabRepo,
		conf.GitlabTargetBranch,
		conf.GitlabPoliciesPath,
		vault.GitlabCurrentCommit,
		r.CommitHash,
	)
	if err != nil {
...
func (r *GitlabRequest) Verify(auth *vault.AuthInfo) error {
	prevProgress := r.Progress

	// fetch a current copy of the diff between github and vault
	reqNow, err := CreateGitlabRequest(auth, map[string]interface{}{
...
func (r *GitlabRequest) Approve(hash string, unsealKey string) error {
	if unsealKey == "" {
...
func (r *GitlabRequest) Reject(auth *vault.AuthInfo, hash string) error {
